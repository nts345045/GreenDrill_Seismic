...
PD_GST = os.path.join(ROOT,'processed_data','Hybrid_Seismic','Corrected_t0','Hiawatha','{SPREAD}','shot','{SHOT}','{SITE}.{SPREAD}.*.mseed')
# File path/name for timing corrected pick data
PD_DTX = os.path.join(ROOT,'processed_data','Hybrid_Seismic','VelCorrected_t0',\
					 'Inglefield_Land','Corrected_Phase_Picks_v5_ele_MK2_pfO3_sutured.csv')
...
for SP_,SH_ in df_META[['Spread','Shot #']].value_counts().index:
	...
	FLIST  = glob(PD_GST.format(SPREAD=SP_,SHOT=SH_,SITE='HI'))
	# Load waveforms
	...
	OPATH = ODIR.format(SITE='HI',SPREAD=SP_,SHOT=SH_)
	try:
		os.makedirs(OPATH)
	except:
		pass
