...
ax2.plot(df_BM_NS['UTM19N mN'],df_BM_NS['Surface Elevation'],'k-',linewidth=3,alpha=0.5)
ax2.plot(df_BM_NS['UTM19N mN'],df_BM_NS['Surface Elevation']-df_BM_NS['Ice Thickness'],'-',linewidth=3,alpha=0.5,color='brown')
...
# Plot Surface elevations from CMP's
# ax1.plot(df_VZN_1_u['mE mean'],df_VZN_1_u['mH mean'],'k.',label='Reference $H_{surf}$')
...
# Plot Surface Elevations for Sensors
df_STWE = df_SITE[df_SITE['Station'].isin(['WE01','WE02','WE03'])]
df_STNS = df_SITE[df_SITE['Station'].isin(['NS01','NS02','NS03'])]
...
# Plot Fir Models
# Hang Uniform Firn model on sensors and shots
mE_WE = list(df_STWE['UTM19N Easting'].values) + list(SHmE) + [4.5e5,4.6e5]
Ht_WE = list(df_STWE['Elevation'].values) + list(df_SHWE['SHOT_elev'].values) + [1402,1383]
S_WE = pd.Series(Ht_WE,index=mE_WE,name='WE surface').sort_index()
mN_NS = list(df_STNS['UTM19N Northing'].values) + list(SHmN) + [8.6e3,8.7e3]
Ht_NS = list(df_STNS['Elevation'].values) + list(df_SHNS['SHOT_elev'].values) + [1388,1383]
S_NS = pd.Series(Ht_NS,index=mN_NS,name='NS surface').sort_index()

ax1.fill_between(S_WE.index,S_WE.values-zRtM,S_WE.values,color='b',alpha=0.1,\
				 label='Firn $\\rho(z)_{Kohnen}\leq 870$ kg m$^{-3}$')
ax2.fill_between(S_NS.index,S_NS.values-zRtM,S_NS.values,color='b',alpha=0.1)
...
# Plot "window-dressing"
ax1.text(Emax+100,1300,'a',fontweight='extra bold',fontstyle='italic',fontsize=16)
ax2.text(Nmax+100,1300,'b',fontweight='extra bold',fontstyle='italic',fontsize=16)


ax1.text(Emin-200,1200,'W',\
	fontweight='extra bold',fontstyle='italic',fontsize=16,color='r',\
	ha='center',va='center')
ax1.text(Emax+50,1200,'E',\
	fontweight='extra bold',fontstyle='italic',fontsize=16,color='r',\
	ha='center',va='center')

ax2.text(Nmin,1200,'S',\
	fontweight='extra bold',fontstyle='italic',fontsize=16,color='r',\
	ha='center',va='center')
ax2.text(Nmax,1200,'N',\
	fontweight='extra bold',fontstyle='italic',fontsize=16,color='r',\
	ha='center',va='center')
